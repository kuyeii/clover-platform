...
当前阶段已进入第 10-B：Portal 能力与竞对分析前端迁入 `apps/web`。`apps/api` 仍是当前主业务后端；`apps/web` 已迁入 Portal 登录、工作台、用户管理、runtime apps、app usage、feedback 和 iframe 容器，并迁入竞对分析真实前端页面；RAG、合同审查、标书生成仍通过 iframe 接入；legacy 前端保留为回滚入口。阶段说明见 `docs/stage-10-b-portal-and-competitor-frontend-migration.md`。
...
当前处于第 10-B 阶段：第 10-A 的统一前端骨架之后，开始把真实平台入口能力迁入 `apps/web`。`apps/web` 已具备 Portal 登录、会话恢复、工作台、四个模块入口、用户管理、runtime apps、app usage、feedback 和 iframe auth bridge；竞对分析真实页面已迁入 `apps/web` 并直接调用 `/api/v1/competitor-analysis/**`。RAG、合同审查、标书生成真实前端本阶段仍保留 iframe；`legacy/portal-launchpad` 和 `legacy/company-competitors-analysis` 保留回滚，不删除 legacy，不去 iframe，`apps/api` 仍是统一后端主应用。阶段边界见 `docs/stage-10-b-portal-and-competitor-frontend-migration.md`。
