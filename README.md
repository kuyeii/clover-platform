...
当前阶段已进入第 10-C：RAG 前端迁入 `apps/web`。`apps/api` 仍是当前主业务后端；`apps/web` 已迁入 Portal、竞对分析和 RAG 真实前端能力；合同审查、标书生成仍通过 iframe 接入；legacy RAG 前端保留为回滚入口。阶段说明见 `docs/stage-10-c-rag-frontend-migration.md`。
...
当前处于第 10-C 阶段：第 10-A 的统一前端骨架和第 10-B 的 Portal / 竞对分析迁移之后，RAG 问答真实前端页面已迁入 `apps/web`。`apps/web` 已具备 Portal 登录、会话恢复、工作台、四个模块入口、用户管理、runtime apps、app usage、feedback、竞对分析真实页面和 RAG 会话 / 聊天流 / knowledge 文档能力。合同审查和标书生成真实前端本阶段仍保留 iframe；legacy RAG 前端保留回滚，不删除 legacy，不去 iframe，`apps/api` 仍是统一后端主应用。阶段边界见 `docs/stage-10-c-rag-frontend-migration.md`。
