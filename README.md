...
当前阶段已进入第 9-C：第 9 阶段继续按模块迁移业务实现，本阶段完成 `contract-review` 合同审查主要业务 API direct 迁移。Portal auth、users、app-usage、runtime apps、feedback 继续使用 `apps/api` 的 `/api/v1/core`；四个业务 iframe 前端仍优先调用对应 `apps/api` 业务入口。合同审查的 health、config、diagnostics、reviews、DOCX document/download 和 AI 改写 API 已由 `apps/api` 直接承载，legacy 合同审查后端暂时保留作为回滚参考，catch-all proxy 仅作为未知路径或临时回滚兜底。当前仍不接 MinIO，不接 Celery / RQ，不新增统一任务表，不修改数据库结构，不改前端请求路径，不影响 competitor-analysis、rag-web-search、bid-generator。阶段说明见 `docs/stage-9-c-contract-review-full-migration.md`。
...
当前处于第 9-C 阶段：在第 9-A 完成竞对分析、第 9-B 完成 RAG 问答主要业务 API direct 迁移后，继续按模块迁移业务实现。本阶段完整迁移合同审查后端主要业务 API 到 `apps/api`，覆盖 health/config/diagnostics、reviews、history/status/result、document/download、风险状态修改和 AI 改写；成功响应保持 legacy-compatible，前端请求路径不变。legacy 合同审查后端暂时保留作为回滚参考，未知路径仍可由 catch-all proxy 兜底。本阶段不接 MinIO，不引入 Celery / RQ，不新增统一任务表，不修改数据库结构，不改业务前端，不影响 competitor-analysis、rag-web-search、bid-generator。阶段边界见 `docs/stage-9-c-contract-review-full-migration.md`。
