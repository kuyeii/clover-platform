...
当前阶段已进入第 8-F：第 7-M 已完成 `competitor-analysis`、`rag-web-search`、`contract-review` 和 `bid-generator` 的 `apps/api` 统一业务代理入口与 iframe auth bridge 总体验收；第 8-A 固化回归基线并稳定开发启动链路；第 8-B 梳理本地文件系统产物边界和现有任务状态边界；第 8-C 完善错误诊断、代理可维护性和本地文件系统版部署准备；第 8-D 新增第一批低风险、只读、无副作用业务 direct API；第 8-E 继续迁移第二批低风险查询类 direct API；第 8-F 对第 8 阶段整体收口。Portal auth、users、app-usage、runtime apps、feedback 使用 `apps/api` 的 `/api/v1/core`；四个业务 iframe 前端均已优先调用对应 `apps/api` 代理入口。当前进入 `apps/api` direct/proxy 混合阶段，复杂业务仍由各自 legacy 后端执行并通过 proxy 透传，继续使用本地文件系统，不接 MinIO，不接 Celery / RQ，不规划任务队列预留口，不规划对象存储预留口，不修改数据库结构。第 9 阶段将在第 8 阶段收口后单独规划。
...
- 未 direct 的复杂链路继续 proxy 到 legacy backend，尤其是 RAG chat stream / knowledge Dataset、竞对分析 analysis / workflows / stream、合同审查 reviews / document / download / AI 改写、标书生成 Dify workflow / SSE task / forge / export / 文件预览下载。
