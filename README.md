...
默认命令会启动配置中 `dev.enabled: true` 的模块。第 4.1-A 已支持自动启动：
...
统一启动器默认使用当前 Python 解释器启动后端。合同审查可通过 `CONTRACT_REVIEW_PYTHON_BIN` 指定解释器；如果未设置且 `legacy/contract_review/.venv/bin/python` 存在，会优先使用该本地解释器，否则回退到当前 Python。
...
`python scripts/dev.py --no-business` 只启动 Portal 前后端。`python scripts/dev.py --only contract-review` 只启动合同审查前后端。动态端口只用于开发环境；Docker 正式部署会在后续部署阶段单独处理。
