...
当前阶段已进入第 10-D：合同审查前端迁入 `apps/web`。`apps/api` 仍是当前主业务后端；`apps/web` 已迁入 Portal、竞对分析、RAG 和合同审查真实前端能力；标书生成仍通过 iframe 接入；legacy 合同审查前端保留为回滚入口。阶段说明见 `docs/stage-10-d-contract-review-frontend-migration.md`。
...
当前处于第 10-D 阶段：第 10-A 的统一前端骨架、第 10-B 的 Portal / 竞对分析迁移、第 10-C 的 RAG 迁移之后，合同审查真实前端页面已迁入 `apps/web`。`apps/web` 已具备 Portal 登录、会话恢复、工作台、四个模块入口、用户管理、runtime apps、app usage、feedback、竞对分析真实页面、RAG 会话 / 聊天流 / knowledge 文档能力，以及合同审查上传、状态轮询、历史记录、风险卡片、AI 改写和 DOCX 下载能力。标书生成真实前端本阶段仍保留 iframe；legacy 合同审查前端保留回滚，不删除 legacy，不去 iframe，`apps/api` 仍是统一后端主应用。阶段边界见 `docs/stage-10-d-contract-review-frontend-migration.md`。
