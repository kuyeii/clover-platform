...
当前阶段已进入第 10-A：统一前端与 `modules` 落位骨架初始化。`apps/api` 仍是当前主业务后端；四个业务前端仍保留在 legacy iframe 链路中；本阶段只建立 `apps/web` 最小骨架、统一 API client 占位、模块路由占位和 `modules` 边界说明，不迁移业务页面，不删除 legacy，不去 iframe。阶段说明见 `docs/stage-10-a-frontend-modules-foundation.md`。
