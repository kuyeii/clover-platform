...
def load_docanalysis_framework(config_path: Path) -> tuple[str, list[dict]]:
...
def build_docanalysis_groups(all_nodes: list[dict], selected_ids: set[str] | None = None) -> list[dict]:
    ...
    groups: list[dict] = []
    first_batch_nodes = [node_by_id[node_id] for node_id in first_batch_ids if node_id in node_by_id]
    second_batch_nodes = [node_by_id[node_id] for node_id in second_batch_ids if node_id in node_by_id]
    if first_batch_nodes:
        groups.append({"group_label": "第一批：项目信息+评分细则+附件结构", "nodes": first_batch_nodes})
    if second_batch_nodes:
        groups.append({"group_label": "第二批：项目技术目标", "nodes": second_batch_nodes})
    return groups


def build_docanalysis_system_prompt(system_prompt_base: str, subset_nodes: list[dict], subset_label: str) -> str:
    node_ids = [str(node.get("id") or "").strip() for node in subset_nodes if str(node.get("id") or "").strip()]
    task_sections = [
        f"### 任务: {str(node.get('label') or '').strip()} (node_id: {str(node.get('id') or '').strip()})\n\n{str(node.get('extractionPrompt') or '').strip()}"
        for node in subset_nodes
        if str(node.get("id") or "").strip() and str(node.get("extractionPrompt") or "").strip()
    ]
...
def extract_docanalysis_text_output(outputs: dict[str, Any]) -> str:
    return str((outputs or {}).get("text") or (outputs or {}).get("result") or (outputs or {}).get("structured_output") or "")
...
def parse_docanalysis_result_map(raw_text: str) -> dict[str, Any]:
    cleaned = re.sub(r"<think>.*?</think>", "", str(raw_text or ""), flags=re.DOTALL).strip()
...
def _normalize_locator(raw_locator: Any) -> str:
    locator = str(raw_locator or "").strip().upper()
    if not locator:
        return ""
    match = re.search(r"P\s*0*(\d+)", locator)
    if not match:
        return locator if locator.startswith("P") else ""
