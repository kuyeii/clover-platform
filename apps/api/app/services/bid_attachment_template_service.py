...
def render_attachment(attachment_type: str, context: dict[str, str]) -> str:
    if attachment_type not in TEMPLATES:
        raise ValueError(f"未知附件类型: {attachment_type}，可选: {list(TEMPLATES.keys())}")
    template = TEMPLATES[attachment_type]
    ...
    rendered = re.sub(
        r"\{\{\s*(?P<key>[a-zA-Z_][a-zA-Z0-9_]*)\s*\|\s*default\('(?P<default>[^']*)'\)\s*\}\}",
    ...
    return re.sub(r"\{\{\s*(?P<key>[a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}", replace_plain, rendered)
