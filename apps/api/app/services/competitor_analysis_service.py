...
MAX_COMPANY_MEMORY_CACHE_SIZE = int(os.getenv("COMPANY_MEMORY_CACHE_SIZE") or 5000)
...
def _load_env_file_once() -> None:
    ...
    with _LEGACY_ENV_LOCK:
        ...
        original_root = get_api_settings().repo_root.parent / "company-competitors-analysis"
        load_dotenv(original_root / ".env", override=False)
        load_dotenv(original_root / ".env.local", override=False)
        _LEGACY_ENV_LOADED = True
...
def _workflow_config() -> dict[str, Any]:
    ...
    value = workflows.get("competitor_analysis")
...
def workflow_url_env_names(*path: str, fallback: list[str]) -> list[str]:
    return [*_config_env_names(*path, field="url_env"), *fallback]


def workflow_key_env_names(*path: str, fallback: list[str]) -> list[str]:
    return [*_config_env_names(*path, field="api_key_env"), *fallback]


def workflow_user_env_names(*path: str, fallback: list[str]) -> list[str]:
    return [*_config_env_names(*path, field="user_env"), *fallback]


def workflow_timeout_env_names(*path: str, fallback: list[str]) -> list[str]:
    return [*_config_env_names(*path, field="timeout_env"), *fallback]
...
def ensure_competitor_storage() -> None:
    try:
        with get_engine().begin() as conn:
    ...
    if missing:
        raise PlatformError(
            ...
            message="竞对分析数据库表不存在。",
            status_code=500,
            details={"tables": [f"competitor_analysis.{name}" for name in missing]},
        )


def _upsert_company_profile_in_connection(
    conn: Connection,
...
class _RepositoryAdapter:
    def ensure_storage(self) -> None:
        ensure_competitor_storage()

    def read_company_validation_query_cache(self, normalized_query: str) -> dict[str, Any] | None:
        try:
            ensure_competitor_storage()
            with get_engine().begin() as conn:
                row = conn.execute(
    ...
    def read_company_profile_cache(self, normalized_name: str) -> dict[str, str] | None:
        try:
            ensure_competitor_storage()
            with get_engine().begin() as conn:
                row = conn.execute(
    ...
    def upsert_company_profile(self, company: dict[str, str], *, normalized_name: str, now_iso: str) -> None:
        try:
            ensure_competitor_storage()
            with get_engine().begin() as conn:
                _upsert_company_profile_in_connection(conn, company, normalized_name=normalized_name, now_iso=now_iso)
        except PlatformError:
            raise
        except SQLAlchemyError as exc:
            raise _database_error(exc) from exc

    def read_validation_response(self, normalized_query: str) -> dict[str, Any]:
        try:
            ensure_competitor_storage()
            with get_engine().begin() as conn:
                row = conn.execute(
        ...
        try:
            ...
            with get_engine().begin() as conn:
                ...
                for company, normalized_name in profiles:
                    _upsert_company_profile_in_connection(
                        conn,
                        company,
                        normalized_name=normalized_name,
                        now_iso=now_iso,
...
def http_post_json(workflow_url: str, body: Dict[str, Any], headers: Dict[str, str], timeout_seconds: Optional[int] = None) -> Tuple[int, str]:
    ...
    timeout = timeout_seconds or get_timeout_seconds(
        workflow_timeout_env_names(fallback=["DIFY_WORKFLOW_TIMEOUT_SECONDS", "WORKFLOW_TIMEOUT_SECONDS"]),
...
def run_input_validation_workflow(
    ...
    **_: Any,
    ...
    workflow_url = get_env(workflow_url_env_names("default", fallback=["WORKFLOW_URL", "VITE_WORKFLOW_URL"]), DEFAULT_WORKFLOW_URL)
    api_key = get_env(workflow_key_env_names("default", fallback=["WORKFLOW_API_KEY", "VITE_WORKFLOW_API_KEY"]))
    workflow_user = get_env(workflow_user_env_names("default", fallback=["WORKFLOW_USER", "VITE_WORKFLOW_USER"]), "admin")
...
def run_company_name_validation_workflow(
    ...
    **_: Any,
    ...
    workflow_url = get_env(
        workflow_url_env_names(
            "company_name_validation",
            fallback=["COMPANY_NAME_VALIDATION_URL", "VITE_COMPANY_NAME_VALIDATION_URL", "WORKFLOW_URL", "VITE_WORKFLOW_URL"],
        ),
        DEFAULT_WORKFLOW_URL,
    )
    api_key = get_env(
        workflow_key_env_names(
            "company_name_validation",
            fallback=["COMPANY_NAME_VALIDATION_API_KEY", "VITE_COMPANY_NAME_VALIDATION_API_KEY"],
        )
    )
    workflow_user = get_env(
        workflow_user_env_names(
            "company_name_validation",
            fallback=["COMPANY_NAME_VALIDATION_USER", "VITE_COMPANY_NAME_VALIDATION_USER", "WORKFLOW_USER", "VITE_WORKFLOW_USER"],
        ),
        "admin",
    )
...
def run_company_detail_workflow(companyName: str = "", companyIntro: str = "", includeRaw: bool = False, **_: Any) -> Any:
    workflow_url = get_env(
        workflow_url_env_names("company_detail", fallback=["COMPANY_DETAIL_URL", "VITE_COMPANY_DETAIL_URL"]),
        DEFAULT_WORKFLOW_URL,
    )
    api_key = get_env(workflow_key_env_names("company_detail", fallback=["COMPANY_DETAIL_API_KEY", "VITE_COMPANY_DETAIL_API_KEY"]))
    workflow_user = get_env(workflow_user_env_names("company_detail", fallback=["COMPANY_DETAIL_USER", "VITE_COMPANY_DETAIL_USER"]), "admin")
    timeout_seconds = get_timeout_seconds(
        workflow_timeout_env_names(
            "company_detail",
            fallback=[
...
def get_compare_workflow_key(step_key: str) -> str:
    ...
    specific = get_env(
        workflow_key_env_names("compare_report", step_key, fallback=[f"COMPARE_REPORT_{upper}_API_KEY", f"VITE_COMPARE_REPORT_{upper}_API_KEY"])
    )
    legacy = get_env(workflow_key_env_names("compare_report", fallback=["COMPARE_REPORT_API_KEY", "VITE_COMPARE_REPORT_API_KEY"]))
...
def get_compare_workflow_url(step_key: str) -> str:
    ...
    return get_env(
        workflow_url_env_names(
            "compare_report",
            step_key,
            fallback=[f"COMPARE_REPORT_{upper}_URL", f"VITE_COMPARE_REPORT_{upper}_URL", "COMPARE_REPORT_URL", "VITE_COMPARE_REPORT_URL"],
        ),
        DEFAULT_WORKFLOW_URL,
    )
...
def run_compare_report_workflow(
    ...
    **_: Any,
    ...
    workflow_user = get_env(workflow_user_env_names("compare_report", fallback=["COMPARE_REPORT_USER", "VITE_COMPARE_REPORT_USER"]), "admin")
    base_inputs = {
...
def run_score_workflow(targetCompany: str = "", reportText: str = "", includeRaw: bool = False, **_: Any) -> Any:
    ...
    workflow_url = get_env(workflow_url_env_names("score", fallback=["SCORE_URL", "VITE_SCORE_URL"]), DEFAULT_WORKFLOW_URL)
    api_key = get_env(workflow_key_env_names("score", fallback=["SCORE_API_KEY", "VITE_SCORE_API_KEY"]))
    workflow_user = get_env(workflow_user_env_names("score", fallback=["SCORE_USER", "VITE_SCORE_USER"]), "admin")
