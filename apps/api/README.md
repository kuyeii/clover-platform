...
`apps/api` 是 Clover Platform 统一后端基座。第 6-E 后 Portal 前端核心平台能力和 feedback 能力默认依赖这里，legacy Portal 后端只作为回滚和过渡兼容保留。第 7-B 起新增竞对分析业务代理试点；合同审查、RAG 和标书生成业务 API 仍由 legacy 后端承载。
...
- 为 `competitor-analysis` 提供鉴权后的 `/api/v1/competitor-analysis/**` 反向代理，目标仍是 legacy 竞对分析后端。
