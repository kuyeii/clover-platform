...
`apps/api` 是 Clover Platform 统一后端基座。第 6-E 后 Portal 前端核心平台能力和 feedback 能力默认依赖这里，legacy Portal 后端只作为回滚和过渡兼容保留。第 7-G 起已接入竞对分析、RAG 和合同审查业务代理；标书生成业务 API 仍由 legacy 后端承载。
...
- 为 `competitor-analysis`、`rag-web-search` 和 `contract-review` 提供鉴权后的业务 API 入口，未直接迁入的业务逻辑仍由对应 legacy 后端执行。
