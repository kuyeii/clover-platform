    ...
    <article className="overflow-hidden rounded-2xl bg-white">
      <div className="flex min-w-0 items-center gap-3 p-4">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          <h3 className="flex h-7 min-w-0 items-center truncate text-base font-normal leading-none text-slate-950">{user.name}</h3>
          <span className="inline-flex h-7 items-center rounded-full bg-slate-100 px-2.5 text-xs font-semibold leading-none text-slate-600">{roleLabelMap[user.role]}</span>
          <span className={["inline-flex h-7 items-center rounded-full px-2.5 text-xs font-semibold leading-none", user.enabled ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"].join(" ")}>
        ...
        >
          <Icon name="arrow" className={["h-4 w-4 transition-transform", isExpanded ? "-rotate-90" : "rotate-90"].join(" ")} strokeWidth={1.7} />
        </button>
      </div>

      <div className={["grid transition-[grid-template-rows] duration-200 ease-out", isExpanded ? "grid-rows-[1fr]" : "grid-rows-[0fr]"].join(" ")}>
        <div className="min-h-0 overflow-hidden">
          <div
            className={["border-t border-slate-100 p-4 pt-3 transition-opacity duration-150", isExpanded ? "visible opacity-100 delay-75" : "invisible opacity-0"].join(" ")}
            aria-hidden={!isExpanded}
          >
          <div className="mb-4 flex flex-col gap-3 rounded-xl bg-slate-50 p-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <p className="text-sm text-slate-500">账号：{user.account} · 最近登录：{formatTime(user.lastLoginAt)}</p>
          ...
          >
            <span className={["grid h-7 w-12 place-items-center rounded-full bg-white text-xs font-semibold shadow-sm transition-transform", user.enabled ? "translate-x-10 text-emerald-700" : "translate-x-0 text-slate-500"].join(" ")}>
              {user.enabled ? "启用" : "停用"}
            </span>
          </button>
            </div>
      ...
      <form className="grid gap-3 lg:grid-cols-[1fr_1fr_auto_auto] lg:items-end" onSubmit={saveAccount}>
        <label className="block">
          ...
          <input className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none transition-colors focus:border-sky-300 focus:ring-2 focus:ring-sky-100" value={name} onChange={(event) => setName(event.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-semibold text-slate-500">账号</span>
          <input className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none transition-colors focus:border-sky-300 focus:ring-2 focus:ring-sky-100" value={account} onChange={(event) => setAccount(event.target.value)} />
        </label>
        <button
          type="button"
        ...
        <button type="submit" disabled={isSaving} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300">
          <Icon name="save" />
          {isSaving ? "保存中" : "保存"}
        </button>
      </form>
          ...
          <div className="mb-5 flex flex-col gap-4 border-b border-slate-100 pb-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <label className="relative block w-full sm:w-72">
                <Icon name="search" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" strokeWidth={1.7} />
                <input
