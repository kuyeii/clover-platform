...
export function LoginPage({ navigate }: { navigate: NavigateFn }) {
  ...
  return (
    <main className="grid min-h-screen place-items-center overflow-hidden bg-slate-50 px-4 py-8 text-slate-950 sm:px-6">
      <section className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70" aria-label="账号登录">
        ...
        <form onSubmit={handleSubmit} className="grid gap-5">
          <label className="grid gap-2">
            ...
            <input
              className="h-12 rounded-lg border border-slate-200 bg-white px-3 text-base text-slate-950 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
              value={account}
          ...
          {formError || error ? (
            <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-base font-medium text-red-700">
              {formError || error}
            </p>
          ) : null}
