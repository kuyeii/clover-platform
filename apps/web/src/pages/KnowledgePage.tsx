...
const statusMeta: Record<KnowledgeStatus, { label: string; className: string }> = {
  enabled: { label: "已启用", className: "bg-emerald-50 text-emerald-700 ring-emerald-100" },
  disabled: { label: "未启用", className: "bg-slate-100 text-slate-700 ring-slate-200" },
  processing: { label: "处理中", className: "bg-sky-50 text-sky-700 ring-sky-100" },
  error: { label: "异常", className: "bg-rose-50 text-rose-700 ring-rose-100" },
};
...
export function KnowledgePage() {
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((item) =>
      [item.name, item.description ?? "", item.data_source_type ?? ""].some((value) => value.toLowerCase().includes(keyword)),
    );
  }, [documents, query]);
  ...
  const handleTextCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(createError instanceof Error ? createError.message : "文本入库失败");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleView = async (documentId: string) => {
    ...
    try {
      ...
      setError(detailError instanceof Error ? detailError.message : "文档详情加载失败");
    } finally {
  ...
  const handleDownload = async (documentId: string) => {
    ...
    try {
      ...
      setError(downloadError instanceof Error ? downloadError.message : "文档下载失败");
    } finally {
  ...
  return (
    <div className="mx-auto min-h-full w-full max-w-7xl space-y-6 px-4 py-6 md:px-8">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={[
              ...
              uploadMode === "file" ? "bg-sky-50 text-sky-700 ring-sky-100" : "bg-white text-slate-600 ring-slate-200 hover:bg-slate-50",
            ].join(" ")}
          >
          ...
          <button
            ...
            className={[
              "inline-flex h-10 items-center gap-2 rounded-lg px-4 text-sm font-medium ring-1",
              uploadMode === "text" ? "bg-sky-50 text-sky-700 ring-sky-100" : "bg-white text-slate-600 ring-slate-200 hover:bg-slate-50",
            ].join(" ")}
          >
        ...
        {uploadMode === "file" ? (
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              type="file"
              disabled={submitting}
              onChange={(event: ChangeEvent<HTMLInputElement>) => void handleFileUpload(event.target.files?.[0] ?? null)}
          ...
          <form className="mt-4 grid gap-3" onSubmit={(event) => void handleTextCreate(event)}>
            <input
              value={textName}
            ...
            >
              <Icon name={submitting ? "refresh" : "upload"} className={["h-4 w-4", submitting ? "animate-spin" : ""].join(" ")} />
              提交入库
            </button>
          </form>
        )}

        {error ? <div className="mt-4 rounded-lg border border-rose-100 bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</div> : null}
      </section>
      ...
      <section className="rounded-2xl border border-white/80 bg-white p-4 shadow-lg md:p-6">
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-sm text-slate-500">
                    <span className="mx-auto mb-2 loading-spinner" />
                    加载中
                  </td>
                </tr>
              ) : null}
                ...
                ? filteredDocuments.map((file) => (
                    <tr key={file.id} className="transition-colors hover:bg-slate-50">
                      <td className="px-5 py-4">
                        ...
                        <div className="mt-1 truncate text-xs text-slate-500">{file.description || file.data_source_type || file.id}</div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-sm text-slate-600">{formatTimestamp(file.updated_at ?? file.created_at)}</td>
                      <td className="px-5 py-4 text-center">
                        <StatusBadge status={resolveStatus(file)} />
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button type="button" className={actionButtonClass} aria-label="查看" disabled={detailLoadingId === file.id} onClick={() => void handleView(file.id)}>
                            <Icon name={detailLoadingId === file.id ? "refresh" : "file"} className={["h-4 w-4", detailLoadingId === file.id ? "animate-spin" : ""].join(" ")} />
                          </button>
                          <button type="button" className={actionButtonClass} aria-label="下载" disabled={busyDocumentId === file.id} onClick={() => void handleDownload(file.id)}>
                            <Icon name="download" className="h-4 w-4" />
                          </button>
                          <button type="button" className={actionButtonClass} aria-label="删除" disabled={busyDocumentId === file.id} onClick={() => void handleDelete(file.id)}>
        ...
        <div className="mt-5 space-y-3 lg:hidden">
          ...
          {!loading && filteredDocuments.length === 0 ? (
            <div className="rounded-xl border border-slate-200 bg-white px-4 py-8 text-center text-sm text-slate-500">暂无文档。</div>
          ) : null}
          {!loading
            ? filteredDocuments.map((file) => (
      ...
      {selectedDetail ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 py-6">
          <section className="max-h-full w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true">
            <div className="flex items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
              <div className="min-w-0">
                <h3 className="truncate text-lg font-semibold text-slate-950">{selectedDetail.document.name || selectedDetail.document.id}</h3>
                <p className="mt-1 text-xs text-slate-500">
                  {selectedDetail.segment_total} 个分段 · {selectedDetail.document.tokens ?? "-"} tokens
                </p>
              </div>
              <button type="button" className={actionButtonClass} aria-label="关闭" onClick={() => setSelectedDetail(null)}>
                <Icon name="close" className="h-4 w-4" />
              </button>
            </div>
            <div className="max-h-[70vh] overflow-auto px-5 py-4">
              <DocumentDetail detail={selectedDetail} />
            </div>
          </section>
        </div>
      ) : null}
    ...
    <div>
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl bg-slate-50 p-3">
          ...
          <p className="mt-1 text-base font-semibold text-slate-900">{document.hit_count ?? "-"}</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-3">
          <p className="text-xs text-slate-500">字数</p>
          <p className="mt-1 text-base font-semibold text-slate-900">{document.word_count ?? "-"}</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-3">
          <p className="text-xs text-slate-500">完成时间</p>
          <p className="mt-1 text-base font-semibold text-slate-900">{formatTimestamp(document.completed_at)}</p>
        </div>
      </div>
      {document.error ? <p className="mt-4 rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{document.error}</p> : null}
      <div className="mt-4 space-y-3">
        {detail.segments.length === 0 ? (
          <p className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">暂无分段</p>
        ) : (
          detail.segments.map((segment, index) => (
            <article key={segment.id ?? `${segment.position}-${index}`} className="rounded-xl border border-slate-200 p-4">
              <div className="mb-2 flex items-center justify-between gap-3 text-xs text-slate-500">
                <span>Segment {segment.position ?? index + 1}</span>
                <span>{segment.tokens ?? "-"} tokens</span>
              </div>
              <p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{segment.content || "（空）"}</p>
            </article>
          ))
        )}
      </div>
