                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input value={recipient} onChange={e => setRecipient(e.target.value)}
                            placeholder="采购人" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:border-brand-500 focus:ring-1 focus:ring-brand-200" />
                    </div>
                    <div>
                        ...
                        <input value={bidNo} onChange={e => setBidNo(e.target.value)}
                            placeholder="如：ZBTB-2025-001" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:border-brand-500 focus:ring-1 focus:ring-brand-200" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">被委托人（委托书专用）</label>
                        <input value={agentName} onChange={e => setAgentName(e.target.value)}
                            placeholder="王五" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:border-brand-500 focus:ring-1 focus:ring-brand-200" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">被委托人身份证号（可选）</label>
                        <input value={agentId} onChange={e => setAgentId(e.target.value)}
                            placeholder="留空则生成下划线占位" className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:border-brand-500 focus:ring-1 focus:ring-brand-200" />
                    </div>
                </div>
                {/* 预填的投标人信息摘要 */}
                {bidderConfigured && (
                        ...
                        <div key={type.key}>
                            ...
                            <div className="px-6 py-3.5 flex items-center gap-4">
                                <div className={clsx('w-8 h-8 rounded-lg flex items-center justify-center shrink-0',
                                    result ? 'bg-[var(--color-success-bg)]' : 'bg-gray-100')}>
                                    {result
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <>
                                            <button onClick={() => handleCopy(type.key, result.content)}
                                                className={clsx('flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors',
                                                    copied ? 'bg-[var(--color-success-bg)] text-success' : 'bg-gray-100 hover:bg-gray-200 text-gray-600')}>
                                    ...
                                    <button onClick={() => handleGenerate(type)} disabled={loading}
                                        className={clsx('flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-colors',
                                            loading ? 'bg-brand-50 text-brand-300 cursor-not-allowed'
                                                : result ? 'bg-white border border-brand-200 text-brand-600 hover:bg-brand-50'
                            ...
                            {/* 错误行 */}
                            {err && (
                                <div className="mx-6 mb-3 text-xs text-danger bg-[var(--color-danger-bg)] border border-[var(--color-danger-border)] px-3 py-2 rounded-lg flex items-center gap-2">
