...
`ContractReviewPage.tsx` 负责编排配置加载、health/diagnostics、上传创建、`run_id` URL 同步、轮询、历史记录、结果刷新、风险状态和 AI 改写操作。`contractReviewApi.ts` 使用统一 `apiClient` 调用 `apps/api`，不再依赖 iframe auth bridge。
...
上传使用 `FormData`，前端不手动设置 multipart `Content-Type`，避免破坏 boundary。上传字段名保持 legacy 行为：`file`、`review_side`、`contract_type_hint`、`analysis_scope`。文件选择层先提示 `.pdf`、`.doc`、`.docx` 之外的格式错误，后端仍保留最终校验。

下载使用统一 API client 的 authenticated raw fetch，自动携带 `Authorization` 和 `X-Portal-Client-Id`，按 blob 保存。文件名优先解析 `Content-Disposition`，无文件名时回退为 `{run_id}.docx` 或当前审查文件名。原始 DOCX 和法务修订文档使用独立就绪状态：`document` 在源 DOCX 可用后允许下载，`download` 仅在审查完成且 `download_ready=true` 后开放，避免处理中误触发修订文档导出。

## 7. AI 改写说明

风险卡片显示 AI 改写状态。单条 `ai_apply`、`ai_accept`、`ai_edit`、`ai_reject` 均有局部 loading；`ai_apply_all` 和 `accept_all` 有整体 loading 和结果提示。`ai_accept` 会把当前改写文本和目标文本一并提交，保持聚合风险和定位类风险与后端 patch 语义一致。Dify / LLM key 未配置或上游失败时按后端业务错误展示，不把 key、prompt 或文件内容写入 UI 或 console。
...
- `/modules/contract-review?run_id=<run_id>` 可恢复状态，completed run 可重新加载结果。
- completed 后可加载 result 和风险卡片。
- 单条风险状态、AI apply / accept / edit / reject 可用。
- apply all / accept all 有清晰 loading 和结果提示。
- document / download 通过鉴权 blob 下载并保留文件名，修订文档按钮只在 `download_ready` 后启用。
