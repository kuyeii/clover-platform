...
RAG 问答模块当前后端主路径已迁入 `apps/api`，真实前端页面已在第 10-C 迁入 `apps/web/src/modules/rag`。第 10-F 后，`apps/web` 是默认前端主入口，`legacy/chat_with_rag_and_websearch/frontend` 默认不启动并继续保留为回滚入口。
...
legacy RAG 前端不删除、不改业务源码，仍可通过原 Vite 项目构建和作为 iframe 回滚入口。需要时使用 `python scripts/dev.py --only rag-web-search --with-legacy-frontends` 启动；如需 legacy 后端回滚，追加 `--with-legacy-backends`。
