...
当前模块 URL 会优先来自 platform-api runtime 接口：

- `GET /api/v1/core/runtime/apps`
- platform-api 读取 `clover-platform/runtime/ports.json`
...
2. 验证 `POST /api/v1/core/auth/login` 成功，`GET /api/v1/core/auth/me` 成功，`core.sessions` 有 session 记录。
...
5. 验证应用占用：
   ...
   - leave 清理占用。
   - `/ws/core/app-usage` WebSocket 仍能推送状态。
