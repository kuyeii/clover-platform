...
也可以通过环境变量覆盖监听地址，单独启动默认仍保持 5000：
...
在 `clover-platform` 根目录使用统一启动器时，标书生成前端和 `pipt-flask` 后端会分别使用 `config/apps.yaml` 中的动态端口范围启动；启动器会向前端注入 `VITE_API_BASE_URL=http://127.0.0.1:<标书生成后端端口>`，前端会自动请求对应的 `<后端>/api`。`gateway-out` 当前是被主后端导入调用的 Python 库/CLI，不是常驻 HTTP 服务，本阶段不单独自动启动。
