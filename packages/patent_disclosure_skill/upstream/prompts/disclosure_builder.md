...
4. 三、本发明技术方案的详细阐述
   ...
   - 3.4 系统流程说明（**必须先写 mermaid 流程图，再写 S1、S2、S3... 或「步骤 S1：...」形式的具体步骤说明**；交付前经工具转 PNG；**不要** ASCII 文字/箭头流程图）
   - 3.4.1 符号与公式（出现公式或形式化变量时**须设**；**须在 3.4 流程图与具体步骤之后**，先符号与变量定义、再写公式；体例见 **§7.7** 与 **`template_reference.md` §3.4.1**）
...
- mermaid 内节点标签用简短中文/数字，避免 ①②③。**3.4 标题下必须先放流程图，再紧接具体步骤说明**；步骤说明使用 `S1：...`、`S2：...` 或 `步骤 S1：...` 形式，对应流程图节点，**不得**用 ASCII 框线箭头代替图示
- 若全文含公式或形式化变量，**不得**让 `3.4.1 符号与公式` 成为 `3.4 系统流程说明` 下第一块内容；必须先完成流程图与 S1—Sn 具体步骤，再进入符号与公式。
