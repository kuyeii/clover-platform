...
- `### 3.4 系统流程说明` 标题下必须先出现本流程图，不能先写 `3.4.1 符号与公式`、参数表或纯概括段落。

#### 流程说明

- 紧接流程图后，用 `S1：...`、`S2：...`、`S3：...` 或 `步骤 S1：...` 的形式写出具体步骤，说明每一步与图中节点的对应关系（**不替代**流程图图示）。
- 步骤说明应覆盖从输入/触发、处理、分支判断、输出/回退的闭环，不只写概括性总结。
- 流程涉及算法、评分、约束或形式化变量时，在流程图与 S1—Sn 步骤之后的 **3.4.1** 集中给出符号定义与主公式；须遵守 **`disclosure_builder.md` §7.7**
