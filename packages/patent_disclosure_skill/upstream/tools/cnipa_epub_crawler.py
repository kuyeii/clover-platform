...
4. **等待首页可检索**：首页在访客到达后会先经 **前端脚本/WAF 一类逻辑**，未通过前 **不会出现** 检索输入框 ``#searchStr``。本实现通过 **周期性轮询 DOM**（每 3 秒一次，总时长见 ``EPUB_WAF_MAX_WAIT_SEC``，默认 300s）直到 ``#searchStr`` 出现；**不是**用 requests 直接 POST 能等价替代的步骤。
...
环境变量
-------------------------------------------------------------------------------
  EPUB_TIMEOUT_MS        首页加载和提交导航等待时间，默认 300000
  EPUB_WAF_MAX_WAIT_SEC  轮询等待 #searchStr 的最长时间，默认 300
...
def _max_wait_sec() -> float:
    return float(os.environ.get("EPUB_WAF_MAX_WAIT_SEC", "300"))


def _timeout_ms() -> int:
    return int(float(os.environ.get("EPUB_TIMEOUT_MS", "300000")))
...
def wait_for_epub_home_ready(page: Page, *, max_wait_sec: float | None = None) -> None:
    ...
    page.goto(EPUB_BASE, wait_until="load", timeout=_timeout_ms())
    elapsed = 0.0
    step = 3.0
    while elapsed < limit:
...
def submit_index_search(page: Page, keyword: str) -> None:
    ...
    for attempt in range(3):
        try:
            page.wait_for_selector("#searchStr", state="attached", timeout=_timeout_ms())
            page.fill("#searchStr", keyword)
            with page.expect_navigation(timeout=_timeout_ms(), wait_until="load"):
                page.evaluate(
                    """() => {
                    const f = document.getElementById('indexForm');
                    if (!f) throw new Error('indexForm not found');
                    f.submit();
                }"""
                )
            ...
            last_err = e
            if not _is_navigation_race(e) or attempt == 2:
                raise
            try:
                page.wait_for_load_state("load", timeout=20_000)
            except Exception:
                pass
            page.wait_for_timeout(800 + 400 * attempt)
    if last_err:
        raise last_err
