...
def _mmdc_extra_args(
    *,
    ...
    if puppeteer_config:
        args.extend(["--puppeteerConfigFile", str(puppeteer_config)])
    return args
...
def _build_puppeteer_config() -> Path | None:
    chromium = next(iter(_candidate_chromium_paths()), None)
    if not chromium:
        return None
    config = {
    ...
    fd, name = tempfile.mkstemp(prefix="mmdc-puppeteer-", suffix=".json")
    with os.fdopen(fd, "w", encoding="utf-8") as fp:
        json.dump(config, fp, ensure_ascii=False)
    path = Path(name)
    print(f"[mermaid_render] 使用 Chromium: {chromium}", file=sys.stderr)
    return path
...
def _render_one_mermaid(
    ...
    *,
    ...
    try:
        extra = _mmdc_extra_args(scale=scale, width=width, height=height, puppeteer_config=puppeteer_config)
        if use_shell:
            parts = [
                *mmdc_base,
                "-i",
...
def render_markdown_mermaid(
    ...
    *,
    ...
    puppeteer_config = _build_puppeteer_config()

    try:
        while i < len(lines):
            line = lines[i]
            if not _MMD_START.match(line):
                out.append(line)
                i += 1
                continue
